...
static int oak_wdt_reboot(PowerOps *me)
{
	uint8_t mode;

	OakPowerOps *power = container_of(me, OakPowerOps, ops);

	mode = power->wdt_mode;

	writel(MTK_WDT_RESTART_KEY, (uint32_t *)MTK_WDT_RESTART);
	 ...
	 */
	udelay(100);
	writel(MTK_WDT_SWRST_KEY, (uint32_t *)MTK_WDT_SWRST);

	halt();
}

static int oak_pass_through_power_off(PowerOps *me)
{
	OakPowerOps *power = container_of(me, OakPowerOps, ops);
...
OakPowerOps *new_oak_power_ops(PowerOps *pass_through, uint8_t mode)
{
	OakPowerOps *power = xzalloc(sizeof(*power));

	power->ops.cold_reboot = &oak_wdt_reboot;
	power->ops.power_off = &oak_pass_through_power_off;
