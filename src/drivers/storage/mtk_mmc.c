...
static void msdc_set_buswidth(MtkMmcHost *host, u32 width)
{
	...
	u32 val = sdr_read32(&reg->sdc_cfg);
...
static void mtk_mmc_change_clock(MtkMmcHost *host, uint32_t clock)
{
	...
	msdc_retry(!(sdr_read32(&reg->msdc_cfg) & MSDC_CFG_CKSTB),
		   retry, cnt, host->id);
	mmc_debug("sclk: %d\n", sclk);
}
...
static void msdc_dma_stop(MtkMmcHost *host)
{
	...
	while (sdr_read32(&reg->dma_cfg) & MSDC_DMA_CFG_STS);
}
					   ...
					   MmcCommand *cmd, MmcData *data)
{
	...
	if (data) {
		...
		sdr_write32(&reg->sdc_blk_num, data->blocks);
	}
	return rawcmd;
}
...
{
	u32 status = sdr_read32(&host->reg->msdc_int);
	if (status)
		sdr_write32(&host->reg->msdc_int, status);
	return status & mask;
}
...
static int msdc_cmd_done(MtkMmcHost *host, int events, MmcCommand *cmd)
{
	...
	if (events & (MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR | MSDC_INT_CMDTMO)) {
		...
		case MMC_RSP_NONE:
			break;
		case MMC_RSP_R2:
			cmd->response[0] = sdr_read32(&reg->sdc_resp3);
			cmd->response[1] = sdr_read32(&reg->sdc_resp2);
			cmd->response[2] = sdr_read32(&reg->sdc_resp1);
			cmd->response[3] = sdr_read32(&reg->sdc_resp0);
			break;
		default:	/* Response types 1, 3, 4, 5, 6, 7(1b) */
			cmd->response[0] = sdr_read32(&reg->sdc_resp0);
			break;
				 ...
				 struct bounce_buffer *bbstate)
{
	...
	sdr_write32(&reg->dma_sa, (uintptr_t) bbstate->bounce_buffer);
	sdr_write32(&reg->dma_length, data->blocks * data->blocksize);
...
{
	MtkMmcReg *reg = host->reg;
	while (sdr_read32(&reg->sdc_sts) & SDC_STS_SDCBUSY) {
		if (timeout == 0) {
				    ...
				    struct bounce_buffer *bbstate)
{
	...
	sdr_write32(&reg->sdc_arg, rawarg);
	sdr_write32(&reg->sdc_cmd, rawcmd);
...
static void msdc_reset_hw(MtkMmcHost *host)
{
	...
	msdc_retry(sdr_read32(&(reg)->msdc_cfg) & MSDC_CFG_RST, retry, cnt, id);
	retry = 3;
	cnt = 1000;
	sdr_set_bits(&(reg)->msdc_fifocs, MSDC_FIFOCS_CLR);
	msdc_retry(sdr_read32(&(reg)->msdc_fifocs) & MSDC_FIFOCS_CLR,
		   retry, cnt, id);
	val = sdr_read32(&(reg)->msdc_int);
	sdr_write32(&(reg)->msdc_int, val);
}
...
static int mtk_mmc_init(BlockDevCtrlrOps *me)
{
	...
	sdr_clr_bits(&reg->msdc_inten, sdr_read32(&reg->msdc_inten));
	sdr_write32(&reg->msdc_int, sdr_read32(&reg->msdc_int));
