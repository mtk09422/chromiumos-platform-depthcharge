...
enum {
	MSDC_CMD_INTS = MSDC_INTEN_CMDRDY | MSDC_INTEN_RSPCRCERR |
			MSDC_INTEN_CMDTMO,
	MSDC_DAT_INTS = MSDC_INTEN_XFER_COMPL | MSDC_INTEN_DATTMO |
...
static inline void sdr_set_bits(uint32_t *reg, uint32_t bs)
{
	u32 val = readl(reg);
	val |= bs;
	writel(val, reg);
...
static inline void sdr_clr_bits(uint32_t *reg, uint32_t bs)
{
	u32 val = readl(reg);
	val &= ~((u32)(bs));
	writel(val, reg);
...
static inline void sdr_set_field(uint32_t *reg, uint32_t field, uint32_t val)
{
        ...
        MmcCommand *cmd, MmcData *data)
{
	...
	rawcmd.u = 0;
	rawcmd.raw.cmd = opcode & 0x3F;
	rawcmd.raw.rsptype = resp & 7;
	...
	if (data) {
		rawcmd.raw.len = data->blocksize & 0xFFF;
...
static int sdc_is_busy(MtkMmcHost *host, MmcCommand *cmd, MmcData *data)
{
	MtkMmcReg *reg = host->reg;
                                    ...
                                    struct bounce_buffer *bbstate)
{
	...
	result = sdc_is_busy(host, cmd, data);
